...
بما أن قواعد Firestore تمنع أي مستخدم من منح نفسه دور `admin` عبر التسجيل
العادي، أول حساب مشرف يجب ضبطه يدويًا من Firebase Console:
...
3. غيّر الحقل `role` من `"user"` إلى `"admin"` واحفظ.
4. سجّل الخروج والدخول مرة أخرى (أو أعد تحميل الصفحة) — ستظهر لك لوحة التحكم.

بعد ذلك، يمكن ترقية أي مستخدم آخر لمشرف من داخل لوحة التحكم نفسها
(`/dashboard/admins`) دون الحاجة للرجوع إلى الكونسول.
